...
4.recommendations"""
        ...
        print(f"Time taken (feedback_narrative): {(end_time - start_time).total_seconds()} seconds")
        credit_narrative_prompt_cost = calculate_prompt_cost(message_prompt, "gpt-4o-2024-08-06")
        credit_narrative_completion_cost = calculate_completion_cost(narrative_chat_completion.choices[0].message.content, "gpt-4o-2024-08-06")
        print(f"Total cost for generating finance narrative = {credit_narrative_prompt_cost + credit_narrative_completion_cost}")
        print("No. of input tokens", count_message_tokens(message_prompt, model="gpt-4o-2024-08-06"))
        print("No. of output tokens", count_string_tokens(prompt=narrative_chat_completion.choices[0].message.content, model="gpt-4o-2024-08-06"))
...
class ScoreStructure(BaseModel):
    score: float = Field(..., description="The score you will generate for the loan application based on the narrative, loan amount, loan-to-collateral ratio and rule based model score provided to you, must be in the range of 0-100")
...
- Extremely Negative Signals: Increase score significantly (e.g., by 25-30 points).

Loan Amount and LTC Ratio Adjustments:
- Low LTC Ratio (<1): Generally low risk, but:
If the loan amount is very small (e.g., under 100000000), the final score should be extremely low (under 30), regardless of other factors.
...
class FeedbackAgent:
    def process_feedback_narrative(self, feedback_text: str) -> str:
        ...
        end_time = datetime.now()
        print(f"Time taken (feedback_narrative): {(end_time - start_time).total_seconds()} seconds")
        feedback_narrative_prompt_cost = calculate_prompt_cost(message_prompt, "gpt-4o-2024-08-06")
        feedback_narrative_completion_cost = calculate_completion_cost(feedback_chat_completion.choices[0].message.content, "gpt-4o-2024-08-06")
        print(f"Total cost for generating finance narrative = {feedback_narrative_prompt_cost + feedback_narrative_completion_cost}")
        print("No. of input tokens", count_message_tokens(message_prompt, model="gpt-4o-2024-08-06"))
        print("No. of output tokens", count_string_tokens(prompt=feedback_chat_completion.choices[0].message.content, model="gpt-4o-2024-08-06"))
    ...
    def process_feedback_note(self, feedback_text: str) -> str:
        ...
        end_time = datetime.now()
        print(f"Time taken (feedback_narrative): {(end_time - start_time).total_seconds()} seconds")
        feedback_note_prompt_cost = calculate_prompt_cost(message_prompt, "gpt-4o-2024-08-06")
        feedback_note_completion_cost = calculate_completion_cost(feedback_chat_completion.choices[0].message.content, "gpt-4o-2024-08-06")
        print(f"Total cost for generating finance narrative = {feedback_note_prompt_cost + feedback_note_completion_cost}")
        print("No. of input tokens", count_message_tokens(message_prompt, model="gpt-4o-2024-08-06"))
        print("No. of output tokens", count_string_tokens(prompt=feedback_chat_completion.choices[0].message.content, model="gpt-4o-2024-08-06"))
        return feedback_chat_completion.choices[0].message.content
...
class CreditNoteAgent:
    ...
    def generate_credit_note(self, narrative: str, user_query: str) -> str:
        ...
        message_prompt = [
            {
                "role": "system",
                "content": f"""Based on the credit narrative provided to you, generate a formal credit note using the provided template. Fill in all sections of the template with appropriate information from the narrative.
                TEMPLATE:
                {self.template}

Replace the placeholder fields like [COMPANY_NAME], [DATE], etc. with actual content.
For [DATE], use the current date.
Make reasonable assumptions for any information not explicitly mentioned in the narrative, but ensure the credit note remains consistent with the narrative's assessment."""
            },
            {
                "role": "user",
        ...
        end_time = datetime.now()
        print(f"Time taken (feedback_narrative): {(end_time - start_time).total_seconds()} seconds")
        credit_note_narrative_prompt_cost = calculate_prompt_cost(message_prompt, "gpt-4o-2024-08-06")
        credit_note_narrative_completion_cost = calculate_completion_cost(credit_note_chat_completion.choices[0].message.content, "gpt-4o-2024-08-06")
        print(f"Total cost for generating finance narrative = {credit_note_narrative_prompt_cost + credit_note_narrative_completion_cost}")
        print("No. of input tokens", count_message_tokens(message_prompt, model="gpt-4o-2024-08-06"))
        print("No. of output tokens", count_string_tokens(prompt=credit_note_chat_completion.choices[0].message.content, model="gpt-4o-2024-08-06"))
...
# ---------------------------------------------------------------------
def main():
    input_data = "Generate me a credit note for a corporate loan for Adani Enterprises Limited"
    ...
    print(risk_score, ltc)
    print("rule based model score:" + str(risk_score) + "\n\n"+ "Loan Amount:" + str(loan) + "\n\n" + "Loan to Collateral Ratio:" + str(ltc))
